...
- [ ] Reset should also re-run tilt calibration when in tilt mode. By the time the user taps Reset they've often repositioned the phone or shifted grip, so the baseline locked in at grant is stale. `resetBall()` should call the same `calibrateAndPlay()` flow Recenter uses (pauses ball, samples 5 events, resumes) — only when mode === 'tilt'
- [ ] Ball still bounces off the board surface despite the ceiling collider being pressed down to ball-diameter + 0.01. Investigate: is the bounce coming from wall collisions (high restitution 0.3 pushing the ball back hard, then up), CCD edge cases at corners, or the ceiling clearance still being too generous? Possible fixes: drop restitution to 0.1, drop ceiling clearance to 0, or zero out ball's Y velocity per frame
...
- [ ] Re-balance ambient lighting on the board. With the bright playground background, the RoomEnvironment env-map, the AmbientLight (0.30), and the dimmed directional (0.70), the wood floor + rocky walls can read as too washed out or too uneven in places. Probably want to retune ambient intensity, the env-map intensity per material (esp. floor/walls so they don't pick up too much sky), or both
